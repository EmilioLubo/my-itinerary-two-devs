...
let Delete = (e)=>{
    ...
    return (
        <div className="w-100 min-h flex j-evenly wrap g-5 p-5 bg-hotel">
            ...
            {hotel.length > 0 ? hotel.map((item) => <Carduser name={item.name}  erase={Delete} photo={item.photo} key={item._id} id={item._id} description={item.capacity} />) : <h2 className="min-h-50">Hotels not found</h2>}
        </div>
    );
}
